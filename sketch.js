...
function draw() {
  ...
  if (scoreParticle !=null){
    ...
    if (scoreParticle.body.position.y > 760){
      ...
      if (scoreParticle.body.position.x >301 && scoreParticle.body.position.x < 600){
        score = score+100;
        scoreParticle = null;
        if (turn >= 5){
          gameState = 1;
      ...
      if (scoreParticle.body.position.x >601 && scoreParticle.body.position.x < 900){
        score = score+200;
        scoreParticle = null;
        if (turn >= 5){
          gameState = 1;
...
function mousePressed(){
  if(gameState !== 1){
    turn++;
    scoreParticle = Particle(mouseX, 10, 10, 10);
  }
}